...
import math
from PySide6.QtWidgets import QGraphicsScene, QGraphicsRectItem, QGraphicsLineItem, QGraphicsTextItem, QGraphicsItem
...
class TimelineScene(QGraphicsScene):
    ...
    def rebuild(self, project, zoom, ph_pos):
        self.clear()
        self._playhead = None
    ...
    def _calc_positions(self, vh, rh):
        ...
        n = len(visible)
        total_gaps = gap * (len(_TRACKS) - 1) if len(_TRACKS) > 1 else 0
        available = vh - rh - 2 - total_gaps - 2
        total_weight = sum(t[3] for t in visible)
        y = rh + 2
        self._track_pos = {}  # só tracks visíveis
        self._label_pos = {}  # todas as tracks (para o painel esquerdo)

        # posições das tracks visíveis na área de conteúdo
        ...
        # posições no painel esquerdo: todas as tracks empilhadas
        lbl_y = rh + 2
        lbl_gap = 4
        lbl_h_expanded = 0
        lbl_h_collapsed = 14
        # calcular altura proporcional para as expandidas no label
    ...
    def _draw_ruler(self, lw, sw, rh, zoom, total):
        item = RulerItem(lw, sw, rh, zoom, total)
        self.addItem(item)
        self._ruler_item = item
    ...
    def _draw_labels(self, lw, vh, rh, project):
        ...
        for key, label, color, _, sub in _TRACKS:
            ...
            eye = _EyeItem(key, 1, cy - 4, 7, not is_collapsed, self.tl)
            self.addItem(eye)

            bar_h = h - 4 if not is_collapsed else h - 2
            bar = QGraphicsRectItem(10, y + 2, 4, bar_h)
            bar.setPen(Qt.NoPen)
            bar.setBrush(QColor(color))
            bar.setZValue(7)
            self.addItem(bar)

            t1 = QGraphicsTextItem(label)
            t1.setFont(QFont("Segoe UI", 7, QFont.Bold))
            t1.setDefaultTextColor(QColor(C["text2"] if not is_collapsed else C["text3"]))
            t1.setPos(16, cy - (12 if not is_collapsed else 6))
            t1.setZValue(7)
            self.addItem(t1)
            ...
            if not is_collapsed:
                ...
                if tk and project and hasattr(project, "track_volumes"):
                    vol = project.track_volumes.get(tk, 1.0)
                    t2 = QGraphicsTextItem(f"{int(vol * 100)}%")
                    t2.setFont(QFont("Consolas", 6, QFont.Bold))
                else:
                    t2 = QGraphicsTextItem(sub)
                    t2.setFont(QFont("Segoe UI", 6))
                t2.setDefaultTextColor(QColor(C["text3"]))
                t2.setPos(16, cy + 1)
                t2.setZValue(7)
                self.addItem(t2)
    ...
    def _draw_clips(self, project, zoom, lw):
        ...
        sel_id = getattr(self.tl, "_selected_clip_id", None)

        for clip in sorted(project.clips, key=lambda c: c.position):
            x = lw + int(t * zoom)
            w = int(clip.duration * zoom)
            is_drag = clip.id == drag_id
            item = ClipGraphicsItem(clip, x, vy, w, vh, selected=is_drag or clip.id == sel_id)
            self.addItem(item)
            if is_drag:
    ...
    def _draw_storyboard(self, project, zoom, lw, vh, rh):
        ...
        for i, scene in enumerate(scenes):
            ...
            x = lw + int(t * zoom)
            line = QGraphicsLineItem(x, rh, x, vh)
    ...
    def _draw_track_items(self, project, zoom, lw):
        sel_id = getattr(self.tl, "_selected_track_item_id", None)
        collapsed = getattr(self.tl, 'collapsed_tracks', set())
        ...
        for name in ("fx", "voice", "sfx", "music", "audio"):
            if name in collapsed:
                continue
            if name not in self._track_pos:
                continue
            ty, th = self._track_pos[name]
            color = color_map[name]
            for ti in sorted(project.get_track_items(name), key=lambda i: i.start_time):
                x = lw + int(ti.start_time * zoom)
                w = max(4, int(ti.duration * zoom))
                if name == "fx":
                    self.addItem(_FxItem(ti, x, ty, w, th, color))
                else:
                    self.addItem(TrackGraphicsItem(ti, x, ty, w, th, color, selected=ti.id == sel_id))

    # ── mouse → interaction ───────────────────────────────────────────────────

    def mousePressEvent(self, event):
        if self._interaction.on_press(event.scenePos(), event.button()):
            event.accept()
        else:
            super().mousePressEvent(event)

    def mouseMoveEvent(self, event):
        if self._interaction.on_move(event.scenePos()):
            event.accept()
        else:
            super().mouseMoveEvent(event)

    def mouseReleaseEvent(self, event):
        if self._interaction.on_release(event.scenePos()):
            event.accept()
        else:
...
class _DiamondItem(QGraphicsItem):
    ...
    def paint(self, painter: QPainter, option, widget=None):
        ...
        if self._hovered:
            ...
            outer = QPolygonF([QPointF(x - sz - 2, cy), QPointF(x, cy - sz - 2),
        ...
        poly = QPolygonF([QPointF(x - sz, cy), QPointF(x, cy - sz),
...
class _StoryboardBadge(QGraphicsItem):
    ...
    def hoverEnterEvent(self, event):
        self._hovered = True; self.setCursor(Qt.PointingHandCursor); self.update()

    def hoverLeaveEvent(self, event):
        self._hovered = False; self.setCursor(Qt.ArrowCursor); self.update()
    ...
    def paint(self, painter: QPainter, option, widget=None):
        ...
        if "fade in" in name:
            ...
            for angle in range(0, 360, 45):
                ...
                painter.drawLine(QPointF(cx, mid),
                                 QPointF(cx + 8 * math.cos(rad), mid + 6 * math.sin(rad)))
        elif "glitch" in name:
            import random
